...
const MintNFT = () => {
    ...
    const uploadMetadataToIPFS = async (imageURL) => {
        ...
        if (!name || !description || !imageURL || !price)
            return console.log("fill complete form");
        const nftJson = {
            name,
            description,
            image: imageURL,
                ...
                <div className="text-white px-10 space-y-2 my-3">
                    <div className="text-start text-white my-3">
                        ...
                        <p className="text-sm my-2">
                            File types supported: JPG, PNG, GIF, MP4, Max size: 20 MB
                        </p>
                    </div>
                    {/* Image Upload */}
                    <div className="">
                    ...
                    {modalMessage.type && (
                        <div
                            className={`w-full h-auto ${modalMessage.type == "success" ? "bg-green-500" : "bg-red-500"
                                } p-1 rounded-lg`}
                        >
                            {modalMessage.message.slice(0, 80)}
                        </div>
                    )}
                    <button
                        disabled={isLoading}
                        className="bg-blue-500 p-3 px-6 flex items-center space-x-2  text-white font-bold rounded-lg text-lg my-3 hover:bg-blue-700"
                        type="submit"
                    >
                        {isLoading ? (
